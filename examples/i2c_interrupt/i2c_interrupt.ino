...
void setup()
{
  ...
  int16_t ret = dps.setInterruptPolarity(1);
  ret = dps.setInterruptSources(1, 0, 0);
  //clear interrupt flag by reading
  dps.getIntStatusFifoFull();
  ...
  int16_t prs_mr = 1;
  int16_t prs_osr = 3;
  ret = dps.startMeasureBothCont(temp_mr, temp_osr, prs_mr, prs_osr);
  if (ret != 0)
  {
...
void onFifoFull()
{
  ...
  dps.getContResults(&temperature[temperatureCount], temp_freespace, &pressure[pressureCount], prs_freespace);
